...
import { DEFAULT_WEIGHTS } from "@/lib/recommendation/criteria";
import type { CriterionWeight } from "@/lib/recommendation/types";
...
/**
 * Catalog-only affordability index retained for legacy card sorting and persistence.
 * Personalized recommendations use price-only min/max normalization in normalization.ts,
 * so performance is not counted twice through the Value criterion.
 */
...
export function computeCameraScore(
  ...
  return clampScore(baselineScore + primaryBoost + ultrawideBoost);
}
...
}: Omit<ScoreBreakdown, "finalScore"> & { weights?: CriterionWeight }) {
  return clampScore(
